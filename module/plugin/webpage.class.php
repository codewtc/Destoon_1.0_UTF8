...
class webpage {
    ...
    function webpage() {
		...
		$this->db = &$db;
		$this->fields = array('item', 'title','style','content','seo_title','seo_keywords','seo_description','editor','edittime','template', 'islink', 'linkurl');
    }
	...
	function add($post) {
		...
		if(!$post['islink']) {
			if($post['filename']) {
				...
				$linkurl = $post['filepath'].'page-'.$this->itemid.'.'.$DT['file_ext'];
			}
