...
<form method="post">
<div class="tt">过期<?php echo $MOD['name'];?></div>
<table cellpadding="2" cellspacing="1" class="tb">
<tr>
...
<?php } ?>
<td><a href="?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=edit&itemid=<?php echo $v['itemid'];?>"><img src="<?php echo IMG_PATH;?>edit.png" width="16" height="16" title="修改" alt=""/></a>&nbsp;
...
天的信息 <input type="submit" value="删 除" class="btn" onclick="if($('days').value==''){alert('请填写天数');return false;}if(confirm('确定要删除过期'+$('days').value+'天的<?php echo $MOD['name'];?>信息吗？此操作将不可撤销')){this.form.action='?moduleid=<?php echo $moduleid;?>&file=<?php echo $file;?>&action=expire&refresh=1&delete=1'}else{return false;}"/></span>
