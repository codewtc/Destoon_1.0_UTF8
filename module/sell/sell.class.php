...
class sell {
    ...
    function sell($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','areaid','typeid','level','title','style','introduce','unit','price','minamount','amount','days','thumb','thumb1','thumb2','tag','keyword','status','username','totime','eidtor','addtime','adddate','edittime','editdate','template','linkurl','note');
    }
	...
	function update($itemid, $r = array(), $content = '') {
		...
		$this->db->query("UPDATE {$this->table} SET linkurl='$linkurl',company='$m[company]',vip='$m[vip]',areaid='$m[areaid]',msn='$m[msn]',qq='$m[qq]' WHERE itemid=$itemid");
		if($this->text_data) {
			if(!$content) {
