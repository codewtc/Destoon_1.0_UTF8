...
class info {
    ...
    function info($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','level','title','style','introduce','totime','areaid','thumb','keyword','name','address','telephone','fax','email','msn','qq','status','username','addtime','eidtor','edittime','template','islink', 'linkurl','note');
    }
	...
	function pass($post) {
		...
		if(!$post['areaid']) return $this->_('请选择地区');
	...
	function set($post) {
		global $MOD, $DT_TIME, $CATEGORY, $AREA, $_username, $_userid;
