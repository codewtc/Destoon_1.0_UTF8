...
switch($action) {
	...
	case 'cate':
		...
		if(!isset($item_count)) {
			...
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE $condition AND status>2");
			$item_count = $r['num'];
	...
	break;
	default:
		$r = $db->get_one("SELECT min(itemid) AS fid,max(itemid) AS tid FROM {$table} WHERE status>2");
		$fid = $r['fid'] ? $r['fid'] : 0;
		$tid = $r['tid'] ? $r['tid'] : 0;
