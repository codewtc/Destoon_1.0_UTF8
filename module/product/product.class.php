...
class product {
    ...
    function product($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','typeid','areaid','level','title','style','introduce','model','standard','brand','unit','price','minamount','amount','days','thumb','thumb1','thumb2','tag','keyword','status','username','eidtor','addtime','adddate','edittime','editdate','template','linkurl','note');
    }
	...
	function set($post) {
		...
		//make keyword
		$post['keyword'] = addslashes($post['tag'].','.$post['title'].','.$post['brand'].','.$post['model'].','.strip_tags(cat_pos($post['catid'], ',')));
		//clear uploads
