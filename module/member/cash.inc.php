...
switch($action) {
	case 'setting':
		if($submit) {
			captcha($captcha);
			$password or message('请填写会员密码');
			is_password($_username, $password) or message('会员密码不正确');
	...
	case 'confirm':
		...
		if($MOD['cash_times']) {
			$r = $db->get_one("SELECT COUNT(itemid) as num FROM {$DT_PRE}finance_cash WHERE username='$_username' AND addtime>$DT_TIME-3600*24");
