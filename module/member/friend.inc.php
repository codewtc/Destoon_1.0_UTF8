...
switch($action) {
	...
	case 'my':
		...
		$r = $db->get_one("SELECT COUNT(itemid) AS num FROM {$DT_PRE}friend WHERE userid=$_userid AND username!=''");
	...
	default:
		...
		$r = $db->get_one("SELECT COUNT(itemid) AS num FROM {$DT_PRE}friend WHERE $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$friends = array();
