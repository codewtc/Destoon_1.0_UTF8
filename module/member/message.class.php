...
class message {
	...
	function send($message) {
		global $MOD, $DT_TIME, $_email, $MODULE, $DT;
		if(!$this->is_message($message)) return false;
		$message['title'] = htmlspecialchars($message['title']);
		$message['content'] = dsafe($message['content']);
		if(isset($message['save'])) {//存草稿
			$this->db->query("INSERT INTO {$this->pre}message(title,typeid,content,fromuser,touser,addtime,status) values('$message[title]','$message[typeid]','$message[content]','$this->username','$message[touser]','$DT_TIME','1')");
		} else {
			...
			foreach(explode(' ', $message['touser']) as $touser) {
				...
				if($user) {
					...
					if(!in_array($this->username, $blacks) && !in_array($touser, $tousers)) { 	$tousers[] = $touser;
						if(isset($message['copy'])) $this->db->query("INSERT INTO {$this->pre}message(title,typeid,content,fromuser,touser,addtime,feedback,status) values('$message[title]','$message[typeid]','$message[content]','$this->username','$touser','$DT_TIME','$feedback','2')");
						$this->db->query("UPDATE {$this->pre}member SET message=message+1 WHERE username='$touser' ");
						$this->db->query("INSERT INTO {$this->pre}message(title,typeid,content,fromuser,touser,addtime,feedback,status) values('$message[title]','$message[typeid]','$message[content]','$this->username','$touser','$DT_TIME','$feedback','3')");
					}
				}
			}
		}
	...
	function get_list($condition, $order = 'itemid DESC') {
		...
		while($r = $this->db->fetch_array($result)) {
			...
			$r['userurl'] = $r['user'] == '系统信使' ? '' : $MODULE[3]['linkurl'].'redirect.php?username='.$r['user'];
			$messages[] = $r;
		}
		return $messages;
	}
	...
	function export($message) {
		...
		if($data) {
			...
			$data = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><meta http-equiv="Content-Type" content="text/html;charset='.$CFG['charset'].'"/><title>'.$this->username.'的'.$names[$status].'信件 '.$DT['sitename'].' '.timetodate($DT_TIME, 5).'导出 - Powered By Destoon.COM</title><style>*{font-size:13px;font-family:Verdana,Arial;}body{width:750px;margin:auto;line-height:200%;}</style><base target="_blank"/><base href="'.DT_URL.'"/><body><br/>'.$data.'<a href="http://www.destoon.com"><small>Powered By Destoon.COM</small></a><br/></body></html>';
			ob_start();
