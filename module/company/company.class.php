...
class company {
	...
	function get_vipt($username) {
		...
		$m = $this->db->get_one("SELECT COUNT(itemid) AS num FROM {$DT_PRE}credit WHERE username='$username' AND status=3");
	...
	function vip_delete($userid) {
		...
		$this->db->query("UPDATE {$this->table_company} SET groupid=5,vip=0,vipr=0,vipt=0,validated=0,validator='',validtime=0 WHERE userid IN ($userids)");
		$this->db->query("UPDATE {$this->table_member} SET groupid=5 WHERE userid IN ($userids)");
