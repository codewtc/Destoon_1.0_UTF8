...
show_menu($menus);
?>
<form method="post" action="?file=<?php echo $file;?>&action=<?php echo $action;?>&userid=<?php echo $userid;?>">
...
<tr>
<td> </td>
<td height="30" colspan="4"><input type="submit" name="submit" value=" 更 新 " class="btn"/> 提示：动作和栏目可按住Ctrl键多选</td>
</tr>
</table>
</form>
<script type="text/javascript">
var html_file = $('file').innerHTML;
var html_action = $('action').innerHTML;
var html_catid = $('catid').innerHTML;
...
function _get_file() {    
	if(xmlHttp.readyState==4 && xmlHttp.status==200) {
		if(xmlHttp.responseText) {
			...
			$('file').innerHTML = s[0] != 0 ? s[0] : html_file;
			$('catid').innerHTML = s[1] != 0 ? s[1] : html_catid;
		}
	}
}
...
function _get_action() {    
	if(xmlHttp.readyState==4 && xmlHttp.status==200) {
		$('action').innerHTML = xmlHttp.responseText != 0 ? xmlHttp.responseText : html_action;
	}
}
</script>
<?php } ?>
