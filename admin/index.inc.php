...
switch($action) {
	case 'cache':
		...
		if($step == 1) {
			...
			cache_clear('cat');
			cache_category();
			msg('栏目缓存更新成功', '?action='.$action.'&step='.($step+1));
		} else if($step == 6) {
			cache_clear('area');
			cache_area();
	...
	case 'password':
		if($submit) {
			...
			if(strlen($password) < 8) msg('新密码最少8位，请修改');
	...
	break;
	case 'side':
		if($DT_TIME - @filemtime(CACHE_ROOT.'/htm/ad.dat') > 12*3600) cache_clear_ad();
		if($DT_TIME - @filemtime(CACHE_ROOT.'/sql/sql.dat') > 2*3600) cache_clear_sql(strtolower(random(2)));
		include tpl('side');
	break;
	case 'main':
		if($submit) {
	...
	case 'count':
		...
		//
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}ask WHERE status=0");
		$num = intval($r['num']);
		echo 'try{document.getElementById("ask").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}finance_charge WHERE status=0");
		$num = intval($r['num']);
		echo 'try{document.getElementById("charge").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}finance_cash WHERE status=0");
		$num = intval($r['num']);
		echo 'try{document.getElementById("cash").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}finance_trade WHERE status=5");
		$num = intval($r['num']);
		echo 'try{document.getElementById("trade").innerHTML="'.$num.'";}catch(e){}';
		//
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}news WHERE status=2");
		$num = intval($r['num']);
		echo 'try{document.getElementById("news").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}credit WHERE status=2");
		$num = intval($r['num']);
		echo 'try{document.getElementById("credit").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}guestbook WHERE status=2");
		$num = intval($r['num']);
		echo 'try{document.getElementById("guestbook").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}link WHERE status=2");
		$num = intval($r['num']);
		echo 'try{document.getElementById("link").innerHTML="'.$num.'";}catch(e){}';
		//
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}member", 'CACHE');
		$num = intval($r['num']);
		echo 'try{document.getElementById("member").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}vip WHERE status=2");
		$num = intval($r['num']);
		echo 'try{document.getElementById("member_vip").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}member WHERE groupid=4");
		$num = intval($r['num']);
		echo 'try{document.getElementById("member_check").innerHTML="'.$num.'";}catch(e){}';

		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}member WHERE regtime>$today");
		$num = intval($r['num']);
		...
		foreach($MODULE as $m) {
			...
			//NEW
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$table} WHERE addtime>$today", 'CACHE', '120');
			$num = intval($r['num']);
