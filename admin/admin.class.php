...
class admin {
	...
	function set_admin($username, $level) {
		...
		if(!$this->is_member($username)) return $this->_('会员不存在');
		$this->db->query("UPDATE {$this->pre}member SET groupid=1,level=$level WHERE username='$username' ");
		return true;
	...
	function delete_admin($username) {
		...
		if($r) {
			...
			if($r['level'] == 1 && $this->count_admin() < 2) return $this->_('系统最少需要保留一位超级管理员');
			$this->db->query("UPDATE {$this->pre}member SET groupid=4,level=0 WHERE username='$username' ");
			$this->db->query("DELETE FROM {$this->pre}admin WHERE userid='$r[userid]' ");
