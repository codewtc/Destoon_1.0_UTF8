...
switch($action) {
	case 'add':
		if($submit) {
			...
			if($post['islink']) {
				...
				if(!is_writeable(DT_ROOT.'/'.$dir)) msg('目录'.$dir.'无法写入，请设置此目录属性为0777');
				if(!file_put(DT_ROOT.'/'.$dir.'/config.inc.php', "DesToon Test")) msg('目录'.$dir.'无法写入，请设置此目录属性为0777');
			}
	...
	case 'remkdir':
		...
		if(is_file($remkdir)) {
			...
			$dir = $r['moduledir'];
			if(!dir_create(DT_ROOT.'/'.$dir)) msg('无法创建'.$dir.'目录，请检查PHP是否有创建权限或手动创建');
			if(!file_put(DT_ROOT.'/'.$dir.'/config.inc.php', "DesToon Test")) msg('目录'.$dir.'无法写入，如果是Linux/Unix服务器，请设置此目录属性为0777');
			file_put(DT_ROOT.'/'.$dir.'/config.inc.php', "<?php\n\$moduleid = ".$moduleid.";\n?>");
			include $remkdir;
