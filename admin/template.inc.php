...
function template_name($fileid = '', $name = '') {
	...
	foreach($names as $k => $v) {
		if(!is_file($template_root.'/'.$k.'.htm')) unset($names[$k]);
	}
...
switch($action) {
	...
	case 'edit':
		if($submit) {
			...
			$fileid or msg();
			if(!is_writeable($template_root.'/'.$fileid.'.htm')) msg($fileid.'.htm不可写，请将其属性设置为0777');
	...
	case 'preview':
		...
		$content = template_parse(stripslashes($content));
		cache_write('_preview.tpl.php', $content, 'tpl');
