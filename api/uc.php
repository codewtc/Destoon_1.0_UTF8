...
switch($action) {
	...
	case 'synlogin':
		...
		$user = $db->get_one("SELECT userid,password,username,groupid,message FROM {$DT_PRE}member WHERE username='$username' limit 0,1");
	...
	break;
	case 'deleteuser':
		$username = $uc_arr['username'];/* 禁止访问 不直接删除 */
		$db->query("UPDATE {$DT_PRE}member SET groupid=2 WHERE username='$username' AND groupid!=1");
		exit('1');
	break;
	case 'updatepw':
		exit('1');//BUG $uc_arr['password'] is Not md5?
		if($uc_arr['password'] && $uc_arr['username']) {
			$password = preg_match('/^\w{32}$/', $uc_arr['password']) ? $uc_arr['password'] : md5($uc_arr['password']);
			$db->query("UPDATE {$DT_PRE}member SET password='$password' WHERE username='$uc_arr[username]'");
			exit('1');
		}
		exit('0');
	break;
