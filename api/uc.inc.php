...
switch($action) {
	case 'login':
		$uc_username = $MOD['uc_charset'] == $CFG['charset'] ? $username : convert($username, $CFG['charset'], $MOD['uc_charset']);
		list($uid, $rt_username, $rt_password, $rt_email) = uc_user_login($uc_username, $password);
		$user = $db->get_one("SELECT username,password,groupid,email FROM {$DT_PRE}member WHERE username='$username' limit 0,1");
		if( $uid == -1 ) {/* Ucenter 用户不存在或被删除 */
			...
			if($user) {
				//$uc_pass = preg_match('/^\w{32}$/', $password) ? $password : md5($password);
				//if($user['password'] != $uc_pass) $db->query("UPDATE {$DT_PRE}member SET password='$uc_pass' WHERE username='$username'");
			} else {/* 会员不存在 */
				$auth = dcrypt($username.'|'.$rt_password.'|'.$rt_email);
				message('请激活您的通行证账号<br/>如果您的用户名不符合本系统规则，请重新注册一个通行证账号', $MOD['linkurl'].'register.php?auth='.$auth, 6);
			}
		}
	break;
	case 'logout':
